...
export default function ReportsPage() {
  ...
  const isAuthorized = user && ['ADMIN', 'SUPPLIER', 'STAFF'].includes(user.role?.toUpperCase());

  const handleGenerateReport = async () => {
    if (!isAuthorized) {
      toast.error('Permission Denied', 'You do not have permission to generate alerts.');
      return;
    }
    
    setGenerating(true);
    try {
      const response = await ProductService.generateReport(user.username, user.role);
      toast.success('Low Stock Alert Sent', 'The notification has been sent successfully to the administrator.');
    } catch (err) {
      toast.error('Report Failed', err.response?.data || err.message || 'Could not trigger report generation.');
    } finally {
      setGenerating(false);
    }
  };
        ...
        <div className="page-inner fade-up">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
            ...
            {isAuthorized && (
              ...
              >
                {generating ? '⌛ Sending...' : '📧 Generate Email Alert'}
              </button>
            )}
          </div>
